...
import { stories, assets, topics, sections, authors, sourceNames } from '../src/data.ts';

test('all bundled content has an English translation without changing its source', () => {
  const content = { stories, assets, topics, sections, authors, sourceNames };
  const before = JSON.stringify(content);
  function check(value) {
    if (typeof value === 'string') {
      assert.equal(/[\u3400-\u9fff]/u.test(translate(value, true)), false, value);
...
test('composed labels and multiline content keep complete phrase translations', () => {
  ...
  assert.equal(translate(stories[0].points.join('\n\n'), true), stories[0].points.map(p => translate(p, true)).join('\n\n'));
